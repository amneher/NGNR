...
const Card = ({ id, image, title, slug, description, teaser, createDate, content, actions, tagIDs, tags }: Article) => {
    ...
    return (
        <div className="card bg-secondary dark:bg-neutral text-base-content dark:text-neutral-content rounded-box border border-transparent shadow-lg  m-2 hover:shadow-slate-950" id={id} aria-label={`card-${id}`} >
            <Link href={`/articles/${slug}`} aria-label={`main-${id}-link`}>
                {image ? <figure><Image src={image} alt="Album" width={default_width} height={default_height} className='rounded-box w-full' aria-label={`image-${id}`} /></figure> : <span aria-label={`noImage-${id}`}></span>}
